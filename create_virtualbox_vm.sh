...
VBoxManage createhd --filename "$(pwd)/$MACHINENAME/$MACHINENAME_sda.vdi" --size 8000 --format VDI
VBoxManage createhd --filename "$(pwd)/$MACHINENAME/$MACHINENAME_sdb.vdi" --size 8000 --format VDI
VBoxManage storagectl $MACHINENAME --name "SATA Controller" --add sata --controller IntelAhci
VBoxManage storageattach $MACHINENAME --storagectl "SATA Controller" --port 0 --device 0 --type hdd --medium "$(pwd)/$MACHINENAME/$MACHINENAME_sda.vdi"
VBoxManage storageattach $MACHINENAME --storagectl "SATA Controller" --port 0 --device 1 --type hdd --medium "$(pwd)/$MACHINENAME/$MACHINENAME_sdb.vdi"
